/**
 * VDA4902 Label Generator Class
 *
 * Mixed load carrier label, Large load carrier label, Container label [small load carrier (KLT)]
 *
 * @link   https://map.myjetbrains.com/youtrack/issue/OTO-34
 * @file   Generator class of VDA4902 label.
 * @author Serkan Mert Kaptan
 * @since  2021-04-19
 */
...
export default class Vda4902 {
    ...
    headings(values) {
        this.text('RECEIVER', '9', 5, 5, 'bold')
            .text('DOCK/GATE', '9', 103, 5, 'bold')
            .text('DELIVERY NOTE NO (N)', '9', 5, 23, 'bold')
            .text('SUPPLIER ADDR', '9', 103, 23, 'bold')
            .text('NET WT (KG)', '9', 103, 36, 'bold')
            .text('GROSS WT (KG)', '9', 139, 36, 'bold')
        ...
        this.text('QUANTITY (Q)', '9', 5, 79, 'bold')
            ...
            .text('DATE', '9', 103, 117.5, 'bold')
            .text('ENGR. CHANGE', '9', 143, 117.5, 'bold')

        if (values.is_mix === '1') {
            this.text('PACK NO (M)', '9', 5, 128, 'bold')
        } else {
            if (values.isMaster === 1) {
                this.text('PACK NO (G)', '9', 5, 128, 'bold')
            } else {
                this.text('PACK NO (S)', '9', 5, 128, 'bold')
            }

        }

        this.text('BATCH NO (H)', '9', 103, 128, 'bold')

        return this
    }

    values(values) {
        values = Vda4902.fixEmpty(values)
        if (values.isMaster === 1) {
            this.text(values.receiverName, '14', 7, 10, 'bold', true)
                ...
                .text(values.dockCode, '64', 104, 19, 'normal', true)
                .text(values.supplierAddr, '20', 104, 30, 'bold', true)
                .text(values.deliveryDocASNNumber, '32', 39, 29, 'bold', true)
        }
        this.text(values.netWeight, '28', 104, 43.5, 'bold', true)
            .text(values.grossWeight, '28', 140, 43.5, 'bold', true)
            .text(values.totalInner, '28', 176, 43.5, 'bold', true)

        if (values.is_mix !== '1') {
            this.text(values.partNumber, '50', 11, 60, 'normal', true)
        } else {
            this.text('MIXED LOAD', '50', 66, 66, 'normal', true)
        }

        this
            .text(values.quantity, '50', 31, 88.5, 'normal', true)
            .text(values.descriptionOfPart, '7mm', 104, 84.5, 'bold', true)
            .text(values.partNumber, '50', 108, 100, 'normal', true)
            .text(values.supplierGsdb, '21', 37, 110.5, 'bold', true)
            .text(values.serialNumber, '21', 37, 129.5, 'bold', true, 'serialNumber')
            .text(values.date, '28', 104, 124, 'bold', true)
            .text(values.engAlert, '28', 145, 124, 'bold', true)
            .text(values.lotBatch, '21', 130, 129.5, 'bold', true)
    ...
    barcodes(values) {
        ...
        this.barcode('Q'.concat(values.quantity), 10, 90)
            ...
            .barcode('M'.concat(values.serialNumber), 10, 130, 13, 'snBarcode')
    ...
    text(string, size, x, y, weight, narrow, id) {
        ...
        text.setAttribute('style', 'font-size:'.concat(size, ';') + 'font-weight:'.concat(weight, ';') + (narrow ? 'font-family:Arial;' : ''))
    ...
    style() {
        ...
        style.innerHTML = 'text{font-family:Arial;}@page{size:auto;margin:0;width:210mm;height:148mm}svg{width:210mm;height:148mm}*{margin:0;}body,html{margin:0;}.label{width:auto;border:0;margin:0 0;padding:0;float:none;position:static;overflow:visible}';
